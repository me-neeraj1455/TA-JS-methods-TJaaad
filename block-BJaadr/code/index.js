...
function everyone() {
  return got.houses.reduce((acc, cv) => {
    acc.push(cv.people.map(n => n.name));
    return acc;
  }, []);
}
...
function nameWithS() {
  return got.houses.reduce((acc, cv) => {
    acc = acc.concat(
      cv.people.map(n => n.name).filter(n => n.toLowerCase().includes(`s`))
    );
    return acc;
  }, []);
}
...
function nameWithA() {
  return got.houses.reduce((acc, cv) => {
    acc = acc.concat(
      cv.people.map(n => n.name).filter(n => n.toLowerCase().includes(`a`))
    );
    return acc;
  }, []);
}
...
function surnameWithA() {
  return got.houses.reduce((acc, house) => {
    acc = acc.concat(
      house.people
        .map(p => p.name)
        .filter(n => n.split(` `)[1].toLowerCase().includes(`a`))
    );
    return acc;
  }, []);
}
...
function peopleNameOfAllHouses() {
  return got.houses.reduce((acc, cv) => {
    acc[cv.name] = cv.people.map(n => n.name);
    return acc;
  }, {});
}
